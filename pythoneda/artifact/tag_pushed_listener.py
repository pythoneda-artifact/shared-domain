"""
pythoneda/artifact/tag_pushed_listener.py
...
from pythoneda.shared.git import GitAdd, GitAddFailed, GitCommit, GitCommitFailed, GitRepo
import requests

class TagPushedListener(Artifact, EventListener):
    """
    ...
    async def update_artifact_version(self, event: TagPushed) -> ArtifactChangesCommitted:
        ...
        if artifact_repo_url is not None:
            ...
            artifact_repo_folder = self.artifact_repository_folder_of(artifact_repo_url, event.repository_folder)
        ...
        if flake is not None:
            ...
            if version_updated:
                hash, change = await self.commit_artifact_changes(artifact_repo_folder, flake, event.repository_url, event.tag)
                if hash:
    ...
    async def commit_artifact_changes(self, artifactRepoFolder: str, flake:str, domainRepoUrl:str, domainTag:str):
        """
        ...
        try:
            ...
            hash, diff = GitCommit(artifactRepoFolder).commit(f"New tag {domainTag} in {domainRepoUrl}")
            repo = GitRepo.from_folder(artifactRepoFolder)
            result = (hash, Change.from_unidiff_text(diff, repo.url, repo.rev, artifactRepoFolder))
