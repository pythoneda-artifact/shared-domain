...
from pythoneda.shared.git import GitAdd, GitAddFailed, GitCommit, GitCommitFailed, GitRepo, GitTag, GitTagFailed, Version
import subprocess
...
class Artifact(BaseObject):
    ...
    async def update_version_in_flake(self, version:str, flake:str) -> bool:
        ...
        try:
            subprocess.run(
                [f"{home_path}/bin/update-sha256-nix-flake.sh", "-f", flake, "-V", version],
                check=True,
        ...
        if self.own_flake(folder):
            ...
            if version_updated:
                try:
                    ...
                    CommittedChangesPushedListener.logger().error('Could not stage changes')
                    CommittedChangesPushedListener.logger().error(err)
                    result = None
                except GitCommitFailed as err:
                    CommittedChangesPushedListener.logger().error('Could not commit staged changes')
                    CommittedChangesPushedListener.logger().error(err)
                    result = None
                except GitTagFailed as err:
                    CommittedChangesPushedListener.logger().error('Could not create tag')
                    CommittedChangesPushedListener.logger().error(err)
                    result = None
        return result
